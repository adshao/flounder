...
- **Governed harness experiments.** `flounder experiment` mines verifier-grounded baseline failures, records passing behavior to preserve, constrains proposals to an explicit harness-file allowlist, and compares paired baseline/candidate Evaluations. Promotion requires distinct cases and bug families, passing hidden holdouts and controls, and no paired regressions. The evaluator, expected answers, safety policy, merge, and deployment authority remain outside the loop.
...
The Evaluations view creates and operates durable audit, benchmark, regression, and verification run groups. Its Harness mode turns finished groups into governed baseline/candidate experiments with mined failure patterns, bounded proposal refinement, compact metric comparison, and an explicit promotion decision. Each item uses a hidden tracking project, and every attempt receives a fresh scope/memory/transcript namespace while sharing only the target dependency cache. Project jobs are claimed ahead of queued Evaluation jobs on a shared daemon; already-running work is never preempted. Lifecycle completion is never presented as an evidence pass by itself, and a promotion recommendation never merges or deploys code.
