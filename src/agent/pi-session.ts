...
- bash: run one local command. Use purpose="inspect" to explore (ls/find/rg/cat). Use purpose="build" for dependency resolution or compilation that makes the workspace buildable (cargo build, cmake -S/-B/--build, ninja, make, forge build, npm install, …); it is not confirmation-eligible. For CMake, prefer generator-neutral commands like cmake -S <src> -B <build> then cmake --build <build> --parallel 2 on large targets; pass -G Ninja only after ninja --version succeeds, and keep parallelism bounded. Use purpose="confirm" to PROVE a bug with a real local test runner (cargo test, ctest, forge test, go test, node --test, pytest, …) and declared success_patterns.
${reportingBlock}
